...
public interface IAuctionRepository extends JpaRepository<Auction, Integer> {
    ...
    @Query(value = "SELECT * FROM auction_web.auction where delete_status = 0", nativeQuery = true)
     ...
     */
    @Query(value = "select * from auction_web.auctio where id in [idList] and delete_status = 0",nativeQuery = true)
